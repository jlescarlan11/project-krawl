...
import { Comment, CommentPageResponse } from "@/types/gem-detail";
...
export function GemComments({ gemId }: GemCommentsProps) {
  ...
  const fetchComments = async (page: number = 0, append: boolean = false) => {
    try {
      ...
      const response = await fetch(`/api/gems/${gemId}/comments?page=${page}&size=20`);
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`/api/gems/${gemId}/comments`, {
        method: 'POST',
