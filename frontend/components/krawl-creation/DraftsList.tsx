 ...
 *
 * Displays a list of saved krawl creation drafts for the user.
...
export function DraftsList() {
  ...
  const loadDrafts = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await listKrawlDrafts();
  ...
  const handleResumeDraft = async (draftId: string) => {
    try {
      ...
      // Navigate to krawl creation page
      router.push("/krawls/create");
    } catch (err) {
  ...
  const handleDeleteDraft = async (draftId: string) => {
    ...
    try {
      ...
      await deleteKrawlDraftApi(draftId);
      // Reload drafts list
      await loadDrafts();
    } catch (err) {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Your saved krawl creation drafts will appear here.
        </p>
      </div>
    );
  }
