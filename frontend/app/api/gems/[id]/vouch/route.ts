/**
 * Vouch API Route Handler
 *
 * Handles POST requests for toggling vouches on gems.
...
export async function POST(
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: "Please sign in to vouch for a gem",
        },
        { status: 401 }
      );
    }
    ...
    const BACKEND_ENDPOINT = `${API_URL}/api/gems/${id}/vouch`;

    console.log(`[POST /api/gems/${id}/vouch] Forwarding to backend: ${BACKEND_ENDPOINT}`);
    ...
    console.log(`[POST /api/gems/${id}/vouch] Backend response status: ${backendResponse.status}`);
    ...
    if (!backendResponse.ok) {
      ...
      try {
        ...
        console.error(`[POST /api/gems/${id}/vouch] Failed to read error response:`, e);
      }
    ...
    console.log(`[POST /api/gems/${id}/vouch] Successfully toggled vouch`, {
    ...
    console.error(`[POST /api/gems/[id]/vouch] Error:`, error);
